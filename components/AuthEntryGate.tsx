  ...
  return (
    <section className="max-w-lg mx-auto px-6 py-16 sm:py-24">
      <Card className="p-7 sm:p-8 text-center">
        <div className={`mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-2xl border text-2xl ${isBattle ? 'border-human/40 bg-human/10' : 'border-[#A78BFA]/40 bg-[#A78BFA]/10'}`}>
          {isBattle ? '⚔️' : 'AI'}
        </div>
        <p className={`text-xs font-mono mb-2 ${isBattle ? 'text-human' : 'text-[#C4B5FD]'}`}>
          {isBattle ? tr('배틀 시작하기', 'Start a battle') : tr('제작자 시작하기', 'Builder sign-in')}
        </p>
        <h1 className="text-2xl sm:text-3xl font-black text-white mb-3">
          {isBattle ? tr('로그인하고 배틀을 시작하세요', 'Sign in to start a battle') : tr('로그인하고 내 도구를 등록하세요', 'Sign in to submit your tool')}
        </h1>
        ...
        <Button size="lg" variant={isBattle ? 'human' : 'primary'} className="w-full" onClick={onLogin}>
