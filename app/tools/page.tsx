  ...
  return (
    <main className="min-h-screen bg-bg">
      <section className="max-w-5xl mx-auto px-6 py-12">
        ...
        <div className="rounded-2xl border border-border bg-surface p-4 sm:p-5 mb-7 space-y-5">
          <div className="flex gap-3 items-end">
            <div className="flex-1">
              <Input label={tr('도구 검색', 'Search tools')} value={query} onChange={event => setQuery(event.target.value)} placeholder={tr('이름, 기능, 분석 방식 검색', 'Search by name, feature, or method')} />
            </div>
            {hasFilters && <Button variant="ghost" onClick={resetFilters}>{tr('초기화', 'Reset')}</Button>}
          </div>

          <FilterRow label={tr('배틀 상태', 'Battle status')} options={[
            ['all', tr('전체', 'All')], ['battle', tr('배틀 가능', 'Battle ready')], ['review', tr('링크·리뷰 전용', 'Review only')],
          ]} value={availability} onChange={value => setAvailability(value as AvailabilityFilter)} />
          ...
          <FilterRow label={tr('지원 시장', 'Markets')} options={[
...
function FilterRow({ label, options, value, onChange }: { label: string; options: string[][]; value: string; onChange: (value: string) => void }) {
  return (
    <div className="grid sm:grid-cols-[88px_1fr] gap-2 sm:items-center">
